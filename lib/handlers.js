...
handlers._users.post = (data, callback) => {
  ...
  const validFirstName = helpers.validateFirstName(firstName);
  const validLastName = helpers.validateLastName(lastName);
  const validPassword = helpers.validatePassword(password);
  const validTosAgreement = helpers.validateTosAgreement(tosAgreement);
...
handlers._users.get = (data, callback) => {
  ...
  if (validPhone) {
   ...
   const validToken = helpers.validateToken(token);
    ...
    handlers._tokens.validateToken(validToken, validPhone, userTokenValidated => {
        ...
        _data.read('users', validPhone, (err, userData) => {
          if (!err && userData) {
            ...
            _data.delete('users', validPhone, err => {
              if (!err){
                const validUserChecks = typeof userData.checks == 'object' && Array.isArray(userData.checks) ? userData.checks : [];
                const checksToDelete = validUserChecks.length;

                if (checksToDelete){
                  let checksDeleted = 0;
                  let deleteErrors = false;
                  // Loop through the checks
                  validUserChecks.forEach(checkId => {
                    // Delete the check
                    _data.delete('checks', checkId, err => {
                      ...
                      if (checksDeleted == checksToDelete){
                        if(!deleteErrors){
                          ...
                          callback(500,{'Error' : 'Not all of the user\'s checks were deleted.'})
                        }
                      }
                    });
                  });
                ...
                callback(500, { 'Error': 'Could not delete provided user.' });
              }
            });
          } else {
            callback(400, { 'Error': 'Could not find provided user.' });
          }
        });
...
handlers._tokens.validateToken = (id, phone, callback) => {
  //Lookup the token
  _data.read('tokens', id, (err, tokenData) => {
    if (!err && tokenData) {
      // Check that token is that of given user and is not expired
      const phoneMatches = tokenData.phone == phone;
      const tokenFresh = tokenData.expires > Date.now();
    ...
    _data.read('checks', validId, (err, checkData) => {
      if (!err && checkData) {
       ...
       const validToken = helpers.validateToken(token);
        ...
        handlers._tokens.validateToken(
          validToken,
          checkData.userPhone,
          userTokenValidated => {
            if (userTokenValidated) {
...
handlers._checks.delete = (data, callback) => {
  // validate check id from querystring
  const { id } = data.queryStringObject || false;
  const validId = helpers.validateId(id);
  ...
  if(validId){
    ...
    _data.read('checks', validId, (err, checkData) => {
      if (!err && checkData){
        ...
        handlers._tokens.validateToken(validToken, checkData.userPhone, tokenIsValid => {
          if (tokenIsValid){
              ...
              if (!err){
                ...
                _data.read('users', checkData.userPhone, (err,userData) => {
                  if (!err){
                    ...
                    var checkPosition = validUserChecks.indexOf(id);
                    if(checkPosition > -1){
                      validUserChecks.splice(checkPosition, 1);
                      ...
                      callback(500, {'Error' : 'Could not find provided check on the user\'s object; Nothing deleted.'});
                    }
                  } else {
                    callback(500, {'Error' : 'Could not find the user who created the check, so deleting check from user checks did not happen.'});
                  }
                });
              } else {
        ...
        callback(400, {'Error' : 'Could not find the provided check ID.'});
      }
    });
  } else {
    callback(400, {'Error' : 'Valid ID required.'});
  }
};
